...
function LoginContent() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Preserve openRequest flag so the service request modal auto-opens
            const openRequest = searchParams.get('openRequest');
            const redirectUrl = openRequest ? `${nextPath}${nextPath.includes('?') ? '&' : '?'}openRequest=true` : nextPath;
    ...
    return (
        <section className="min-h-screen pt-28 pb-20 flex items-center justify-center relative overflow-hidden bg-surface-950">
            ...
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[900px] h-[500px] rounded-full pointer-events-none opacity-15"
                style={{ background: 'radial-gradient(ellipse, rgba(20,184,166,0.25) 0%, transparent 70%)' }} />
            <div className="absolute bottom-0 right-0 w-[400px] h-[400px] rounded-full pointer-events-none opacity-10"
                style={{ background: 'radial-gradient(circle, rgba(34,211,238,0.2) 0%, transparent 70%)' }} />

            {/* Floating orbs */}
            <motion.div
                className="absolute top-20 left-10 w-2 h-2 bg-brand-400/40 rounded-full pointer-events-none"
                animate={{ y: [0, -20, 0], opacity: [0.4, 0.8, 0.4] }}
                transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }}
            />
            <motion.div
                className="absolute bottom-32 right-20 w-3 h-3 bg-accent-400/30 rounded-full pointer-events-none"
            ...
            <div className="section-container relative z-10 w-full max-w-[440px] mx-auto">
                    ...
                    <Card className="card-glow p-8 sm:p-10 border-surface-800/80 shadow-[0_25px_60px_rgba(0,0,0,0.5)] bg-surface-900/60 backdrop-blur-2xl">
                        <CardContent className="p-0">
                            ...
                            <div className="text-center mb-8">
                                ...
                                >
                                    <Lock size={26} className="text-brand-400 relative z-10" />
                                </motion.div>
                            ...
                            {message && (
                                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
                                    <Alert className="mb-6 bg-brand-500/10 border-brand-500/20 text-brand-400">
                            ...
                            <div className="space-y-3 mb-6">
                                ...
                                >
                                    ...
                                    <span>{oauthLoading === 'azure' ? 'Connecting...' : 'Continue with Microsoft'}</span>
                                </Button>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-5">
                                ...
                                <div className="space-y-1.5">
                                    <div className="flex items-center justify-between ml-1 mr-1">
                                        ...
                                        <Link href="/login" className="text-xs text-brand-400 hover:text-brand-300 transition-colors" tabIndex={-1}>
                                            Forgot password?
                                        </Link>
                                    </div>
                                    <div className="relative input-glow rounded-md">
                                ...
                                <Button
                                    type="submit"
                                    disabled={loading}
                                    className="w-full h-12 bg-brand-500 hover:bg-brand-400 text-surface-950 font-bold tracking-wide uppercase btn-glow btn-shine mt-2"
                                    size="lg"
                                >
                                    {loading ? (
                ...
                {/* Trust badge */}
                <motion.p
                    className="text-center text-surface-600 text-xs mt-6 flex items-center justify-center gap-1.5"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
